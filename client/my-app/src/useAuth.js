...
import axios from "axios"; //axios is used to call our api which is the alternative to built in fetch api i think
...
export default function useAuth(code) { //will store access, refresh, and expiresin
    ...
    //This is printing our expires in and tokens to the console, but I'm not sure how the login is exactly working with the code
    ...
    useEffect(() => {
        axios.post('http://localhost:3001/login', {
            ...
            window.location = '/' //this is redirecting the person back to the login page in case tokens expire but I'm not sure how
        })
    }, [code])
    ...
    useEffect(() => {
        ...
        because it's thinking that the refreshtoken is being changed so this should not be allowed to run unless
